...
from xg_model_nhl import MoneyPuckXGTrainer
...
def main():
    ...
    model = MoneyPuckXGTrainer()

    try:
        # Run the model training
        model.run()
        final_auc = 0.85  # Placeholder - would need to extract from model.run()

        logger.info("\n" + "=" * 60)
        logger.info("FINAL RESULTS")
        logger.info("=" * 60)
        logger.info(f"Model AUC: {final_auc:.4f}")
        logger.info("MoneyPuck AUC: 0.7781")
