"""
Build complete xG feature set combining all available data sources
Implements the missing features identified in the analysis
"""
...
def extract_player_ids_from_string(player_string: str) -> List[int]:
    """Extract player IDs from string like '["Connor McDavid (8478402)", "Leon Draisaitl (8477934)"]'"""
    ...
    for player in players:
        # Extract ID from "Player Name (ID)" format
        if "(" in player and ")" in player:
            player_id = int(player.split("(")[1].split(")")[0])
            ids.append(player_id)
    return ids


def calculate_on_ice_quality(
    offensive_ids: List[int], defensive_ids: List[int], player_tiers: pd.DataFrame, player_stats: pd.DataFrame
) -> Dict:
    """Calculate quality metrics for players on ice"""

    # Get player data
    off_players = player_tiers[player_tiers["player_id"].isin(offensive_ids)]
    def_players = player_tiers[player_tiers["player_id"].isin(defensive_ids)]
